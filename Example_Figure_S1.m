...
plot(1e9*x2/M,I2,pm(i));
hold on;
end

set(gca,'XLim',[-100 100],'FontSize',14);
xlabel('x [nm]');
ylabel('Normalized intensity');

% Make a legend
p = 1e9*dx./m;
p = cellfun(@(x) [num2str(x,3) ' nm'],num2cell(p),'UniformOutput',false);
legend(p);
...
if s == 1
    ...
    print('Figure_S1.pdf','-dpdf','-r0');
    print('Figure_S1.png','-dpng','-r600');
end
