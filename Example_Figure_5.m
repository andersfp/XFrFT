...
% Make object
w = 600e-6; % width of square object
E0 = rect(X0./w).*rect(Y0./w);
...
subplot(2,2,1);
plot(x2,l2,'-',x70,l70,'--');
set(gca,'XLim',[-5e-3 5e-3],'YLim',[0 0.01]);
...
yyaxis left;
plot(x2,l2 - l70);
set(gca,'XLim',[-5e-3 5e-3],'YLim',[-6e-7 6e-7]);
title('Detector intensity error');
xlabel('x [m]');
ylabel('Absolute error [a.u.]');
yyaxis right;
plot(x2,(l2 - l70)./l70);
set(gca,'XLim',[-5e-3 5e-3],'YLim',[-0.01 0.01]);
ylabel('Relative error');
subplot(2,2,3);
plot(x2,p2,'-',x70,p70,'--');
set(gca,'XLim',[-5e-3 5e-3],'YLim',[0 3.5e5]);
...
yyaxis left;
plot(x2,p2 - p70);
set(gca,'XLim',[-5e-3 5e-3],'YLim',[-0.35 0.35]);
